...
class ControllerPaymentBamboraOnlineCheckout extends Controller
{
    ...
    {
        $this->language->load('payment/'.$this->module_name);
        ...
        $this->response->setOutput($this->load->view('payment/'.$this->module_name . '.tpl', $this->data));
    ...
    protected function populateBreadcrumbs()
    {
        ...
        if($this->is_oc_3()) {
            ...
            $this->data['breadcrumbs'][] = array(
                'text' => $this->language->get('heading_title'),
                'href' => $this->url->link('payment/'.$this->module_name, 'token=' . $this->session->data['token'], 'SSL'),
            );
            $this->data['action'] = $this->url->link('payment/'.$this->module_name, 'token=' . $this->session->data['token'], 'SSL');
    ...
    {
        if (!$this->user->hasPermission('modify', 'payment/'.$this->module_name)) {
    ...
    public function install()
    {
        ...
        $this->model_payment_bambora_online_checkout->install();
    }
    ...
    public function getPaymentTransaction()
    {
        ...
        if ($moduleStatus && isset($this->request->get['order_id'])) {
            ...
            if ($dbTransaction && !empty($dbTransaction['transaction_id'])) {
                $transactionResponse = $this->model_payment_bambora_online_checkout->getTransaction($dbTransaction['transaction_id']);
                ...
                if ($transactionResponse && $transactionResponse->meta->result) {
                    ...
                    $authorizedAmount = $this->model_payment_bambora_online_checkout->convertPriceFromMinorunits($transaction->total->authorized, $transaction->currency->minorunits, $decimalPoint, $thousandSeparator);
                    ...
                    $surchargeFeeAmount = $this->model_payment_bambora_online_checkout->convertPriceFromMinorunits($transaction->total->feeamount, $transaction->currency->minorunits, $decimalPoint, $thousandSeparator);
                    $data['transaction']['surchargeFee'] = "{$transaction->currency->code} {$surchargeFeeAmount}";

                    $capturedAmount = $this->model_payment_bambora_online_checkout->convertPriceFromMinorunits($transaction->total->captured, $transaction->currency->minorunits, $decimalPoint, $thousandSeparator);
                    $data['transaction']['captured'] = "{$transaction->currency->code} {$capturedAmount}";

                    $redundedAmount = $this->model_payment_bambora_online_checkout->convertPriceFromMinorunits($transaction->total->credited, $transaction->currency->minorunits, $decimalPoint, $thousandSeparator);
                    ...
                    if (isset($transaction->information->ecis)) {
                        $data['transaction']['eci'] = $this->model_payment_bambora_online_checkout->getLowestECI($transaction->information->ecis);
                    }
                    if (isset($transaction->information->exemptions)) {
                        $data['transaction']['exemptions'] = $this->model_payment_bambora_online_checkout->getDistinctExemptions($transaction->information->exemptions);
                    }

                    $availableForCapture = $this->model_payment_bambora_online_checkout->convertPriceFromMinorunits($transaction->available->capture, $transaction->currency->minorunits);
                    $data['transaction']['availableForCapture'] = $availableForCapture;
                    $availableForRefund = $this->model_payment_bambora_online_checkout->convertPriceFromMinorunits($transaction->available->credit, $transaction->currency->minorunits);
                    ...
                    $transactionOperationsResponse = $this->model_payment_bambora_online_checkout->getTransactionOperations($dbTransaction['transaction_id']);
        ...
        $this->response->setOutput($this->load->view('payment/'.$this->module_name.'_order_ajax.tpl', $data));
    }
    ...
    protected function createTransactionOperations($transactionOperation, $decimalPoint, $thousandSeparator)
    {
        ...
        foreach ($transactionOperation as $operation) {
            $eventText = $this->model_payment_bambora_online_checkout->getEventText($operation);
            if ($eventText['description'] != null){
                $eventInfoExtra = '';

                if ($operation->status != 'approved'){
                    $eventInfoExtra = $this->model_payment_bambora_online_checkout->getEventExtra($operation);
                ...
                $operationAmount = $this->model_payment_bambora_online_checkout->convertPriceFromMinorunits($operation->amount, $operation->currency->minorunits, $decimalPoint, $thousandSeparator);
    ...
    public function capture()
    {
        ...
        try {
            ...
            $minorunits = $this->model_payment_bambora_online_checkout->getCurrencyMinorunits($currencyCode);
            $rawAmount = $postParams['captureAmount'];
            $sanitizedAmount = str_replace(',', '.', $rawAmount);
            $amount = (float)$sanitizedAmount;
            $amountMinorunits =  $this->model_payment_bambora_online_checkout->convertPriceToMinorunits($amount, $minorunits);
            $captureResponse = $this->model_payment_bambora_online_checkout->capture($transactionId, $amountMinorunits, $currencyCode);
            ...
            if (isset($captureResponse)) {
                ...
                $this->model_payment_bambora_online_checkout->bamboraLog('Capture Failed: ' . $errorMessage);
            }
        }
        catch (Exception $ex) {
            $json['meta']['result'] = false;
            $json['meta']['message']['merchant'] = $ex->getMessage();
            $this->model_payment_bambora_online_checkout->bamboraLog($ex->getMessage());
        }
    ...
    public function refund()
    {
        ...
        try {
            ...
            $amount = (float)$sanitizedAmount;
            $amountMinorunits =  $this->model_payment_bambora_online_checkout->convertPriceToMinorunits($amount, $minorunits);
            $refundResponse = $this->model_payment_bambora_online_checkout->refund($transactionId, $amountMinorunits, $currencyCode);
            ...
            if (isset($refundResponse)) {
                ...
                $errorMessage = "Connection to Bambora Failed";
                $json['meta']['message']['merchant'] = $errorMessage;
                $this->model_payment_bambora_online_checkout->bamboraLog('Refund Failed: ' . $errorMessage);
            }
        }
        catch (Exception $ex) {
            $json['meta']['result'] = false;
    ...
    public function void()
    {
        ...
        try {
            ...
            $postParams = $this->request->post;
            $transactionId = $postParams['transactionId'];
            $voidResponse = $this->model_payment_bambora_online_checkout->void($transactionId);
            ...
            if (isset($voidResponse)) {
                ...
                $errorMessage = "Connection to Bambora Failed";
                $json['meta']['message']['merchant'] = $errorMessage;
                $this->model_payment_bambora_online_checkout->bamboraLog('Void Failed: ' .$errorMessage);
            }
        }
        catch (Exception $ex) {
            $json['meta']['result'] = false;
