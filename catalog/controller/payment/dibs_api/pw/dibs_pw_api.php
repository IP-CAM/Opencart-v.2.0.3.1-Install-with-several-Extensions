...
class dibs_pw_api extends dibs_pw_helpers {
    ...
    private function api_dibs_commonFields(&$aData, $oOrder) {
        ...
        if((string)$this->helper_dibs_tools_conf('testmode') == 'yes') {
        	$aData['test'] = 1;
	}
        if( isset($this->session->data['payment_method']['dibspw_paytype']) && 
                $this->session->data['payment_method']['dibspw_paytype'] == 'card') {
            $aData['paytype'] = $this->helper_dibs_tools_conf('card_paytype');
        }
        if(isset($this->session->data['payment_method']['dibspw_paytype']) 
                && $this->session->data['payment_method']['dibspw_paytype'] == 'invoice' ) {
            $aData['paytype'] = $this->helper_dibs_tools_conf('invoice_paytype');
        }
