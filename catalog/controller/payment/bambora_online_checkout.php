 ...
 *
 */
class ControllerPaymentBamboraOnlineCheckout extends Controller
{
    /**
     * @var string
     */
    ...
    public function index()
    {
        ...
        return $this->load->view('default/template/payment/'.$this->module_name . '.tpl', $data);
    }
    ...
    {
        $this->load->model('checkout/order');
        $this->load->model('payment/bambora_online_checkout');
        $this->load->language('payment/' . $this->module_name);

        $checkoutSessionRequest = $this->createCheckoutSessionRequest();
        $checkoutSessionResponse = $this->model_payment_bambora_online_checkout->setCheckoutSession($checkoutSessionRequest);
        ...
        if (!$checkoutSessionResponse || $checkoutSessionResponse->meta->result == false) {
            ...
            $this->model_payment_bambora_online_checkout->bamboraLog($this->language->get('error_payment_window') . ' ' . $checkoutSessionResponse->meta->message->merchant);
        } else {
    ...
    {
        $this->language->load('payment/bambora_online_checkout');
        $this->load->model('checkout/order');
    ...
    public function callback()
    {
        ...
        try {
            ...
            if (!$this->validateCallback($getParameteres, $message, $transaction)) {
                ...
                $this->model_payment_bambora_online_checkout->bamboraLog($errorMessage);
                header('X-EPay-System: ' . $this->model_payment_bambora_online_checkout->getModuleHeaderInformation(), true, 500);
                die($errorMessage);
            }

            //Lock for multiple callbacks on already confirmed payment
            $orderId = $transaction->orderid;
            $dbTransaction = $this->model_payment_bambora_online_checkout->getDbTransaction($orderId);
            ...
            if (($dbTransaction && !empty($dbTransaction['transaction_id'])) || $orderInfo['order_status_id'] === $this->config->get($this->getConfigBaseName() . '_order_status_completed')) {
                header('X-EPay-System: ' . $this->model_payment_bambora_online_checkout->getModuleHeaderInformation(), true, 200);
            ...
            $amount = $this->model_payment_bambora_online_checkout->convertPriceFromMinorunits($transaction->total->authorized, $minorunits, $decimalPoint, $thousandSeparator);
            ...
            $this->model_payment_bambora_online_checkout->addDbTransaction($orderId, $transaction->id, $transaction->total->authorized, $transaction->currency->code);
    ...
    protected function getAllowedPaymentTypeIds($currency, $amount, $orderId)
    {
        ...
        $minorunits = $this->model_payment_bambora_online_checkout->getCurrencyMinorunits($currency);
        $amountInMinorunits = $this->model_payment_bambora_online_checkout->convertPriceToMinorunits($amount, $minorunits);
        $paymentTypeResponse = $this->model_payment_bambora_online_checkout->getPaymentTypeIds($currency, $amountInMinorunits);
    ...
    protected function createCheckoutSessionRequest()
    {
        ...
        $orderInfo = $this->model_checkout_order->getOrder($this->session->data['order_id']);
        $minorunits = $this->model_payment_bambora_online_checkout->getCurrencyMinorunits($orderInfo['currency_code']);
        $orderTotalAmount = 0;
        $orderTaxAmount = 0;
        $orderTotals = array();
        if($this->is_oc_3()) {
        ...
        $params['order']['amount'] = $this->model_payment_bambora_online_checkout->convertPriceToMinorunits($orderTotalAmount, $minorunits);
        $params['order']['vatamount'] = $this->model_payment_bambora_online_checkout->convertPriceToMinorunits($orderTaxAmount, $minorunits);
        ...
        $params['url']['accept'] = $this->url->link('payment/bambora_online_checkout/accept', '', true);
        $params['url']['cancel'] = $this->url->link('checkout/checkout', '', true);
        $params['url']['callbacks'] = array();
        $params['url']['callbacks'][] = array('url' => $this->url->link('payment/' . $this->module_name . '/callback', '', true));


        $allow_low_value = $this->config->get($this->getConfigBaseName() . '_allow_low_value_exemptions');
        if ( $allow_low_value){
            if ($params['order']['amount'] < $this->model_payment_bambora_online_checkout->convertPriceToMinorunits($this->config->get($this->getConfigBaseName() . '_limit_for_low_value_exemption'), $minorunits)){
    ...
    protected function createOrderLines($orderTotals, $minorunits)
    {
        ...
        foreach ($orderProducts as $product) {
            ...
            $line['totalprice'] = $this->model_payment_bambora_online_checkout->convertPriceToMinorunits($lineTotalPrice, $minorunits);
            $line['totalpriceinclvat'] = $this->model_payment_bambora_online_checkout->convertPriceToMinorunits($lineTotalPriceInclVat, $minorunits);
            $line['totalpricevatamount'] = $this->model_payment_bambora_online_checkout->convertPriceToMinorunits($lineTaxAmount, $minorunits);
            $line['unitprice'] = $this->model_payment_bambora_online_checkout->convertPriceToMinorunits($lineTotalPrice/$product['quantity'], $minorunits);
            $line['unitpriceinclvat'] = $this->model_payment_bambora_online_checkout->convertPriceToMinorunits($lineTotalPriceInclVat/$product['quantity'], $minorunits);
            $line['unitpricevatamount'] = $this->model_payment_bambora_online_checkout->convertPriceToMinorunits($lineTaxAmount/$product['quantity'], $minorunits);
        ...
        if (isset($shipping) && isset($this->cart->session->data['shipping_method'])) {
            ...
            if (!empty($shippingMethod['cost']) && $shippingMethod['cost'] > 0) {
                ...
                $shipping['totalprice'] = $this->model_payment_bambora_online_checkout->convertPriceToMinorunits($shippintTotalPrice, $minorunits);
                $shipping['totalpriceinclvat'] = $this->model_payment_bambora_online_checkout->convertPriceToMinorunits($shippingWithTax, $minorunits);
                $shipping['totalpricevatamount'] = $this->model_payment_bambora_online_checkout->convertPriceToMinorunits($shippingTaxAmount, $minorunits);
                $shipping['unitprice'] = $this->model_payment_bambora_online_checkout->convertPriceToMinorunits($shippintTotalPrice, $minorunits);
                $shipping['unitpriceinclvat'] = $this->model_payment_bambora_online_checkout->convertPriceToMinorunits($shippingWithTax, $minorunits);
                $shipping['unitpricevatamount'] = $this->model_payment_bambora_online_checkout->convertPriceToMinorunits($shippingTaxAmount, $minorunits);
        ...
        if (!empty($orderTotalDiscount) && (float)$orderTotalDiscount['value'] < 0) {
            ...
            $coupon['totalprice'] = $this->model_payment_bambora_online_checkout->convertPriceToMinorunits($cuponTotalPrice, $minorunits);
            $coupon['totalpriceinclvat'] = $this->model_payment_bambora_online_checkout->convertPriceToMinorunits($cuponTotalPrice, $minorunits);
            $coupon['totalpricevatamount'] = 0;
            $coupon['unitprice'] = $this->model_payment_bambora_online_checkout->convertPriceToMinorunits($cuponTotalPrice, $minorunits);
            $coupon['unitpriceinclvat'] = $this->model_payment_bambora_online_checkout->convertPriceToMinorunits($cuponTotalPrice, $minorunits);
        ...
        if (!empty($orderTotalVoucher) && (float)$orderTotalVoucher['value'] < 0) {
            ...
            $voucher['totalprice'] = $this->model_payment_bambora_online_checkout->convertPriceToMinorunits($voucherTotalPrice, $minorunits);
            $voucher['totalpriceinclvat'] = $this->model_payment_bambora_online_checkout->convertPriceToMinorunits($voucherTotalPrice, $minorunits);
            $voucher['totalpricevatamount'] = 0;
            $voucher['unitprice'] = $this->model_payment_bambora_online_checkout->convertPriceToMinorunits($voucherTotalPrice, $minorunits);
            $voucher['unitpriceinclvat'] = $this->model_payment_bambora_online_checkout->convertPriceToMinorunits($voucherTotalPrice, $minorunits);
    ...
    protected function validateCallback($getParameters, &$message, &$transaction)
    {
        ...
        $transactionResponse = $this->model_payment_bambora_online_checkout->getTransaction($transactionId);
    ...
    {
        $this->load->language('total/bambora_online_checkout_fee');
        ...
        $transactionFee = $this->model_payment_bambora_online_checkout->convertPriceFromMinorunits($baseTransactionFee, $transaction->currency->minorunits);
