...
class ModelCatalogInformation extends Model {
	public function addInformation($data) {
		...
		$information_id = $this->db->getLastId();
			...
			foreach ($data['information_description'] as $language_id => $value) {
				...
				if(intval($this->config->get('seo_pp_auto_information_seo_url'))==1){
					$page_title = trim($this->db->escape($value['title']));
					...
					$page_seo_keyword = str_replace(array(
						...
						$information_id,
						strlen($page_title) == 0 ? '' : trim($page_title),
					),
						$this->config->get('seo_pp_information_seo_url'));

					$data['keyword'] = $this->model_seo_power_pack_settings->url_slug(trim($page_seo_keyword));
				...
				if(intval($this->config->get('seo_pp_auto_information_meta_title'))==1){
					$data['information_description'][$language_id]['meta_title'] = str_replace(array(
						'[ title ]',
					), array(
						strlen(trim($value['title'])) == 0 ? '' : trim($value['title']),
					),
						$this->config->get('seo_pp_information_meta_title'));
				}

				if(intval($this->config->get('seo_pp_auto_information_meta_desc'))==1){
					$description = $this->model_seo_power_pack_settings->clean_string($value['description']);

					$data['information_description'][$language_id]['meta_description'] = str_replace(array(
						'[ title ]',
						'[ description ]',
					), array(
						strlen(trim($value['title'])) == 0 ? '' : trim($value['title']).' ',
						...
						$this->config->get('seo_pp_information_meta_desc'));
				}
			}
		...
		if (false && intval($this->config->get('seo_pp_auto_information_seo_url'))==0 && isset($data['keyword'])) {
	...
	public function editInformation($information_id, $data) {
		...
		if (false && intval($this->config->get('seo_pp_auto_information_seo_url'))==0 && $data['keyword']) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET query = 'information_id=" . (int)$information_id . "', keyword = '" . $this->db->escape($data['keyword']) . "'");
		}
	...
	public function getInformation($information_id) {
		$query = $this->db->query("SELECT DISTINCT *, (SELECT keyword FROM " . DB_PREFIX . "url_alias WHERE query = 'information_id=" . (int)$information_id . "' AND language_id IN(0,".(int)$this->config->get('config_language_id').") LIMIT 1) AS keyword FROM " . DB_PREFIX . "information WHERE information_id = '" . (int)$information_id . "'");
